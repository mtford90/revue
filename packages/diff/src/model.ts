...
import type { DiffFile, DiffFileInput, DiffHunk, DiffMetadata, DiffStats } from "./types.ts";
...
export function countDiffStats(metadata: DiffMetadata): DiffStats {
	let additions = 0;
	let deletions = 0;
...
const fromPierreHunk = (hunk: FileDiffMetadata["hunks"][number]): DiffHunk => ({
...
/** Keep the upstream parser representation behind Revue's structural model boundary. */
const fromPierreMetadata = (metadata: FileDiffMetadata): DiffMetadata => ({
	name: metadata.name,
	prevName: metadata.prevName,
	newObjectId: metadata.newObjectId,
	prevObjectId: metadata.prevObjectId,
	mode: metadata.mode,
	prevMode: metadata.prevMode,
	type: metadata.type,
	hunks: metadata.hunks.map(fromPierreHunk),
...
/** Internal Pierre conversion used only by the syntax adapter. Not exported from the package. */
export const toPierreMetadata = (metadata: DiffMetadata, language: string): FileDiffMetadata => ({
	name: metadata.name,
	prevName: metadata.prevName,
	lang: language,
	newObjectId: metadata.newObjectId,
...
export function parsePatch(patchText: string, sourceId = "patch"): DiffFile[] {
	...
	return patchChunks(normalizedPatch).flatMap((chunk, chunkIndex) =>
		parsePatchFiles(ensureGitHeader(chunk), `${sourceId}:${chunkIndex}`, true)
			...
			.map((parsedMetadata) => {
				const metadata = fromPierreMetadata(patchMetadata(parsedMetadata, chunk));
