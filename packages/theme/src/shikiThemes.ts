// The bundled Shiki themes Revue derives its palettes from. Each entry records only what a
// terminal palette needs — the editor surface, the editor foreground, and the theme's own
// semantic diff colours — so no Shiki theme JSON is loaded to paint the shell.
...
const BACKGROUNDS: Record<BundledShikiThemeId, string> = {
	andromeeda: "#23262e",
	"aurora-x": "#07090f",
	"ayu-dark": "#10141c",
	"ayu-light": "#fcfcfc",
...
const FOREGROUNDS: Partial<Record<BundledShikiThemeId, string>> = {
	andromeeda: "#d5ced9",
	"ayu-dark": "#bfbdb6",
	"ayu-light": "#5c6166",
	"ayu-mirage": "#cccac2",
...
const DIFF_COLORS: Partial<Record<BundledShikiThemeId, BundledShikiThemeDiffColors>> = {
...
export const isBundledShikiThemeId = (
	themeId: string | undefined,
): themeId is BundledShikiThemeId =>
	BUNDLED_SHIKI_THEME_IDS.includes(themeId as BundledShikiThemeId);

export const bundledThemeBackground = (themeId: BundledShikiThemeId): string =>
	BACKGROUNDS[themeId];

export const bundledThemeForeground = (themeId: BundledShikiThemeId): string | undefined =>
	FOREGROUNDS[themeId];
