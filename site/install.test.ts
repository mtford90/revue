...
describe.each(cases)("$name installer release resolution", (installer) => {
	test("paginates until the latest product-specific stable exact-semver release", async () => {
		...
		await writeFile(`${bin}/curl`, mockCurl(sha, archiveName));
		await chmod(`${bin}/curl`, 0o755);

		// releases/latest redirects to a tag belonging to the sibling product,
		// forcing the fallback to the paged API.
		const child = Bun.spawn({
			...
			env: {
				...
				MOCK_LATEST_TAG: `${wrongPrefix}9.0.0`,
	...
	test("resolves the latest tag from the releases/latest redirect without calling the API", async () => {
		const root = await mkdtemp(`${tmpdir()}/revue-installer-`);
		temporaryDirectories.push(root);
		const bin = `${root}/bin`;
		const install = `${root}/install`;
		...
		expect(exitCode).toBe(0);
		expect(stdout).toContain(`installed ${executableName} ${installer.version}`);
		expect(await Bun.file(`${root}/curl.log`).text()).not.toContain("api.github.com");
	});
});

function mockCurl(sha: string, archiveName: string): string {
	return `#!/bin/sh
set -eu
url=""
out=""
...
case "$url" in
  */releases/latest)
    case "$write_fmt" in
      *url_effective*) printf '%s' "https://github.com/mtford90/revue/releases/tag/$MOCK_LATEST_TAG" ;;
    esac
    ;;
  *api.github.com*releases*page=1)
    cat "$MOCK_ROOT/page-1.json" > "$out"
    case "$write_fmt" in *http_code*) printf '200' ;; esac
    ;;
  *api.github.com*releases*page=2)
    cat "$MOCK_ROOT/page-2.json" > "$out"
    case "$write_fmt" in *http_code*) printf '200' ;; esac
    ;;
